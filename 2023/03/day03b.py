...
def checkAdjacentCells(p_currRow, p_currCol):
            ...
            if (row != p_currRow) or (col != p_currCol):
                ...
                if (row, col) not in partsMapping:
                    ...
                    if adjacentCell.isdigit():
                        ...
                        adjacentNums.append(numberFullString)
    ...
    if len(adjacentNums) == 2:
        gearLocation = (p_currRow, p_currCol)
        gearsMapping[gearLocation] = adjacentNums
...
for coords in gearsMapping:
    gearRatio = int(gearsMapping[coords][0]) * int(gearsMapping[coords][1])
    totalCalibrationValue += gearRatio
